...
function findProjectPbxProjPath(cwd) {
...
function modifyBundleReactNativeShellScript(phase) {
    ...
    if (shellScriptContentLines.includes(bundleCommand)) {
        logger.info(`${phase.name} phase in project.pbxproj already contains ${bundleCommand}`);
        return phase;
    }
...
function modifyPbxprojConfig(pbxprojPath) {
 ...
 */
export default function modifyIOS(cwd) {
    ...
    if (!fs.existsSync(projectPbxProjPath)) {
        throw Error(`${relativeProjectPbxProjPath} not found. Make sure you are running this command from the root of your project`);
    }
    ...
    logger.success(`Added "webpack-bundle" as BUNDLE_COMMAND to build phase shellScript in ${relativeProjectPbxProjPath}`);
}
