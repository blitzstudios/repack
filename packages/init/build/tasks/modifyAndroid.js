...
function modifyOldConfig(config) {
    ...
    const applyReactGradleRegex = /(?<!`)apply from: "\.\.\/\.\.\/node_modules\/react-native\/react\.gradle"/;
    ...
    if (reactConfigIndex === -1) {
        ...
        return config.replace(applyReactGradleRegex, defaultOldConfig +
            '\n\n' +
    ...
    if (bundleCommandRegex.test(config)) {
        ...
        return config.replace(bundleCommandRegex, 'bundleCommand: "webpack-bundle"');
    }
    // Otherwise, add the bundleCommand to the existing config
    return config.replace(projectExtReactRegex, 'project.ext.react = [\n    bundleCommand: "webpack-bundle",');
}
function modifyNewConfig(config) {
    logger.info('Modifying android/app/build.gradle for React-Native >= 0.71.0');
    const androidConfig = 'android {';
    const reactConfigIndex = config.indexOf('react {');
    if (reactConfigIndex === -1) {
        // Add the default react config if it doesn't exist
        return config.replace(androidConfig, defaultNewConfig + '\n\n' + androidConfig);
    }
    ...
    if (existingBundleCommandRegex.test(config)) {
        ...
        return config.replace(existingBundleCommandRegex, 'bundleCommand = "webpack-bundle"');
    }
    // Otherwise, add the bundleCommand to the existing config
 ...
 */
export default function modifyAndroid(cwd, reactNativeVersion) {
    const buildGradlePath = path.join(cwd, 'android', 'app', 'build.gradle');
    const config = fs.readFileSync(buildGradlePath, 'utf-8');
    if (!fs.existsSync(buildGradlePath)) {
        throw Error('File android/app/build.gradle not found. Make sure you are running this command from the root of your project');
    }
    ...
    logger.success('Added "webpack-bundle" as bundleCommand to android/app/build.gradle');
}
