...
export class WebSocketMessageServer extends WebSocketServer {
    ...
    async openDevTools(clientId) {
        try {
            ...
            const devtoolsFrontendUrl = data.find((item) => item.devtoolsFrontendUrl)?.devtoolsFrontendUrl;
            if (devtoolsFrontendUrl) {
                ...
                const chromeFrontendUrl = `https://chrome-devtools-frontend.appspot.com${devtoolsFrontendUrl}`;
                await open(chromeFrontendUrl, { app: { name: 'google chrome' } });
                this.fastify.log.info({
