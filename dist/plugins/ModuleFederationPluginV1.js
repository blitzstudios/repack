...
class ModuleFederationPluginV1 {
  ...
  replaceRemotes(remote) {
    if (typeof remote === 'string') {
      return remote.startsWith('promise new Promise') ? remote : _utils.Federated.createRemote(remote);
    }
  ...
  getDefaultSharedDependencies() {
    return {
      react: _utils.Federated.SHARED_REACT,
      'react-native': _utils.Federated.SHARED_REACT_NATIVE
    };
  }
  ...
  adaptSharedDependencies(shared) {
    const sharedDependencyConfig = (eager, version) => ({
      singleton: true,
      eager: eager ?? true,
      version: version || '*',
    ...
    if (Array.isArray(shared)) {
      ...
      if (!findSharedDependency('react-native/', shared)) {
        adjustedSharedDependencies.push({
          'react-native/': sharedDependencyConfig(reactNativeEager, reactNativeVersion)
        });
      }
      if (!findSharedDependency('@react-native/', shared)) {
        adjustedSharedDependencies.push({
...
//# sourceMappingURL=ModuleFederationPluginV1.js.map