...
class LoggerPlugin {
  ...
  constructor(config) {
    ...
    const isTruthyEnv = env => {
      return !!env && env !== 'false' && env !== '0';
    };
    const reporters = [];
    if (this.config.output.console) {
      reporters.push(new _index.ConsoleReporter({
        isWorker: isTruthyEnv(process.env[_env.WORKER_ENV_KEY]),
        isVerbose: isTruthyEnv(process.env[_env.VERBOSE_ENV_KEY])
      }));
    }
    if (this.config.output.file) {
      reporters.push(new _index.FileReporter({
        filename: this.config.output.file
      }));
    }
    this.reporter = (0, _index.composeReporters)(reporters);
  }
  ...
  apply(compiler) {
    ...
    compiler.hooks.done.tap('RepackLoggerPlugin', stats => {
      if (compiler.options.devServer) {
        ...
        let entires = [];
        if (errors?.length) {
          entires = [this.createEntry('LoggerPlugin', 'error', ['Failed to build bundle due to errors']), ...errors.map(error => this.createEntry('LoggerPlugin', 'error', [`Error in "${error.moduleName}":\n${error.message}`]))];
        } else {
          entires = [this.createEntry('LoggerPlugin', 'info', [warnings?.length ? 'Bundle built with warnings' : 'Bundle built', {
            time
          }]), ...(warnings?.map(warning => this.createEntry('LoggerPlugin', 'warn', [`Warning in "${warning.moduleName}":\n${warning.message}`])) ?? [])];
        }
