...
class AssetsCopyProcessor {
  ...
  enqueueChunk(chunk, {
    ...
    const sourceMapDestination = isEntry ? sourcemapOutput : _path.default.join(platform === 'ios' ? assetsDest : sourcemapOutputDir, sourceMapFile !== null && sourceMapFile !== void 0 ? sourceMapFile : ''); // Entry chunks (main/index bundle) need to be processed differently to
    ...
    const mediaAssets = [...chunk.auxiliaryFiles].filter(file => !/\.(map|bundle\.json)$/.test(file));
    ...
    if (manifest) {
      ...
      if (isEntry) {
        this.queue.push(async () => {
          ...
          await this.filesystem.writeFile(manifestDestination, manifestContent.replace(chunkFile, _path.default.basename(bundleDestination)).replace(sourceMapFile !== null && sourceMapFile !== void 0 ? sourceMapFile : /.^/, _path.default.basename(sourceMapDestination)));
        });
      } else {
