...
function getFilesInDirectory(dirname, fs) {
  return new Promise((resolve, reject) => fs.readdir(dirname, (error, results) => {
    if (error) {
      reject(error);
    } else {
      resolve(results?.filter(result => typeof result === 'string') ?? []);
    }
  }));
}
