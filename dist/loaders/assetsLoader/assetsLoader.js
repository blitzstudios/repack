...
async function repackAssetsLoader(assetData) {
  ...
  try {
    ...
    // defaults
    const scalableAssetExtensions = options.scalableAssetExtensions ?? _utils.SCALABLE_ASSETS;
    const scalableAssetResolutions = options.scalableAssetResolutions ?? _utils.SCALABLE_RESOLUTIONS;
    ...
    const scales = await (0, _utils2.collectScales)(resourceAbsoluteDirname, resourceFilename, resourceExtensionType, scalableAssetExtensions, scalableAssetResolutions, readDirAsync);
    const scaleKeys = Object.keys(scales).sort((a, b) => (0, _utils2.getScaleNumber)(a) - (0, _utils2.getScaleNumber)(b));
    ...
    const assets = await Promise.all(scaleKeys.map(async scaleKey => {
      ...
      let destination;
      if (!options.devServerEnabled && !options.remote?.enabled && options.platform === 'android') {
        // found font family
      ...
      const scale = (0, _utils2.getScaleNumber)(scaleKey);
      const dimensions = (0, _utils2.getAssetDimensions)({
